...
namespace planning
{
	namespace rbt
	{
		...
		protected:
			...
			double delta = M_PI/2;                          // Radius of hypersphere from q to q_e
			uint numSpines = 7;                             // Number of bur spines

			double get_d_c(std::shared_ptr<base::State> q);
			...
			std::tuple<planning::rrt::Status, std::shared_ptr<base::State>> extendSpine(std::shared_ptr<base::State> q, std::shared_ptr<base::State> q_e);
			planning::rrt::Status connectSpine(std::shared_ptr<base::Tree> tree, std::shared_ptr<KdTree> kdtree, std::shared_ptr<base::State> q, std::shared_ptr<base::State> q_e);
